...
int tcpListen(struct sockaddr_in *addr, uint16_t port) {
    ...
    if (fd != -1) {
        int opt = 1;
        //calling system call bind and listen
        if (-1 != assert("setsockpt", setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof opt)) &&
...
//this will be used to send output to client sockets
